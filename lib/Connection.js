...
class Connection {
  ...
  doRequest({ url, method, data }) {
    ...
    if (this.config.sessionToken) {
      _.extend(fetchOptions.headers, {
        'X-Auth-Token': this.config.sessionToken
      });
    }
  ...
  handleResponse(response) {
    ...
    logger.debug('handleResponse:', { status });
    ...
    return response.text()
      .then(text => {
        logger.debug('handleResponse:', { text });
        ...
        logger.debug('handleResponse:', { json });
  ...
  handleLoginResponse(response) {
    ...
    return response.text().then(text => {
      if (text) {
        if (status < 400) {
          const json = JSON.parse(text);
          if (json && json.data && json.data.user && json.data.token) {
            return Promise.resolve(json.data.token);
          }
        }
      }
      return Promise.reject(new ConnectionError(status, 'invalid response'));
    })
  }
